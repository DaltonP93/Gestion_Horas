...
function buildPushStatusPayload(data = {}) {
  ...
  return {
    ...
    matched_by: found ? (data.matchedBy || MATCHED_BY.NONE) : MATCHED_BY.NONE,
  };
}
...
function validatePushStatusPayload(payload) {
  ...
  if (!isIsoOrNull(payload.last_push_at)) return { ok: false, reason: 'last_push_at no es una fecha ISO' };
  if (!isIsoOrNull(payload.last_event_at)) return { ok: false, reason: 'last_event_at no es una fecha ISO' };
  return { ok: true };
}
